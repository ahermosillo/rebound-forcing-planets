...
void reb_update_acceleration(struct reb_simulation* r){
	...
	if (r->additional_forces  && (r->integrator != REB_INTEGRATOR_MERCURIUS || r->ri_mercurius.mode==0)){
		...
		// printf("additional forces calculated in kick step not closer encounter\n");
        if (r->integrator==REB_INTEGRATOR_MERCURIUS){
			...
			// printf("shift pos and velocity so that external forces are calculated in intertial frame\n");
