...
void reb_integrator_mercurius_interaction_step(struct reb_simulation* const r, double dt){
    ...
    for (int i=1;i<N;i++){
        particles[i].vx += dt*particles[i].ausrx;
        particles[i].vy += dt*particles[i].ausry;
        particles[i].vz += dt*particles[i].ausrz;
		particles[i].x  += dt*particles[i].vusrx;
        particles[i].y  += dt*particles[i].vusry;
        particles[i].z  += dt*particles[i].vusrz;
	     }
...
void reb_integrator_mercurius_part2(struct reb_simulation* const r){
    struct reb_simulation_integrator_mercurius* const rim = &(r->ri_mercurius);
    const int N = r->N;
    //printf("mercurius part 2\n");
    //printf("time, %20.16e\n", r->t*(365.25/(2*M_PI)));

    if (rim->is_synchronized){
        // printf("is synchronized\n");
