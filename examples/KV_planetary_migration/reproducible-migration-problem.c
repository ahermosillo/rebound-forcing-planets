...
int main(int argc, char* argv[]){
   ...
   if (r==NULL){
      ...
      double* M0 = calloc(sizeof(double),4);
      ...
      struct reb_particle pJ = reb_tools_orbit_to_particle(r->G, r->particles[0],masspl[0],a0[0],e0[0],inc0[0],node0[0],w0[0],M0[0]);
      pJ.r = rpl[0];
      pJ.hash = 1;
      struct reb_particle pS = reb_tools_orbit_to_particle(r->G, r->particles[0],masspl[1],a0[1],e0[1],inc0[1],node0[1],w0[1],M0[1]);
      pS.r = rpl[1];
      pS.hash = 2;
      struct reb_particle pU = reb_tools_orbit_to_particle(r->G, r->particles[0],masspl[2],a0[2],e0[2],inc0[2],node0[2],w0[2],M0[2]);
      pU.r = rpl[2];
      pU.hash = 3;
      struct reb_particle pN = reb_tools_orbit_to_particle(r->G, r->particles[0],masspl[3],a0[3],e0[3],inc0[3],node0[3],w0[3],M0[3]);
      pN.r = rpl[3];
      pN.hash = 4;
      reb_add(r, pJ); 
      reb_add(r, pS); 
...
void migration_forces(struct reb_simulation* r){
   ...
   struct reb_particle com = r->particles[0]; // calculate migration forces with respect to the sun;
   for(int i=1;i<N;i++){
      if (tau_a[i]!=0){
